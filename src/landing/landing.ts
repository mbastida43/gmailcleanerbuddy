...
const HTML_KEYS = new Set<string>([
  'step2_d', 'step3_d',
  'arch2_d', 'arch3_d',
...
const langMenu = document.getElementById('langMenu') as HTMLElement | null;
const curFlag = document.getElementById('curFlag') as HTMLElement | null;
const curLabel = document.getElementById('curLabel') as HTMLElement | null;

function applyI18n(code: string): void {
...
function setLang(code: string): void {
  ...
  if (curFlag) curFlag.textContent = l.flag;
  if (curLabel) curLabel.textContent = l.label;
  applyI18n(code);
  if (langMenu) {
    langMenu.querySelectorAll<HTMLButtonElement>('button').forEach((b) => {
      b.setAttribute('aria-selected', String(b.dataset['code'] === code));
    });
  }
  try { localStorage.setItem('gcb_lang', code); } catch (e) { /* storage disabled */ }
}
